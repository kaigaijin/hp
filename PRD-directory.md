...
```
/{country}/place/                     → カテゴリ一覧ページ
/{country}/place/{category}/          → そのカテゴリの店舗一覧
/{country}/place/{category}/{slug}    → 店舗詳細ページ
```
...
### カテゴリ一覧 `/{country}/place/`
- カテゴリカードのグリッド表示（アイコン + カテゴリ名 + 件数）
- 国のヒーロー（国旗・国名）

### 店舗一覧 `/{country}/place/{category}/`
- カテゴリ内の全店舗をカード表示
- エリアでのフィルタリング（将来）
- 各カードに: 店名・エリア・タグ・概要

### 店舗詳細 `/{country}/place/{category}/{slug}`
...
- [x] 1.3 カテゴリ一覧ページ（`[country]/place/page.tsx`）
- [x] 1.4 店舗一覧ページ（`[country]/place/[category]/page.tsx`）
- [x] 1.5 店舗詳細ページ（`[country]/place/[category]/[slug]/page.tsx`）
