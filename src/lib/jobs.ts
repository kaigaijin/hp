...
// 業種別求人を取得
export function getJobsByIndustry(countryCode: string, industry: string): Job[] {
  const filePath = path.join(jobsDir, countryCode, `${industry}.json`);
...
export function getJob(
  ...
  const jobs = getJobsByIndustry(countryCode, industry);
  return jobs.find((j) => j.slug === slug);
}

// 国の全求人を取得
export function getAllJobs(countryCode: string): Array<Job & { industry: string }> {
  return JOB_INDUSTRIES.flatMap((ind) =>
    getJobsByIndustry(countryCode, ind.slug).map((job) => ({
      ...job,
      industry: ind.slug,
    })),
  );
...
// 業種別件数を取得
export function getIndustryCounts(countryCode: string): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const ind of JOB_INDUSTRIES) {
    counts[ind.slug] = getJobsByIndustry(countryCode, ind.slug).length;
  }
  return counts;
}
